...
% For neon green dot
hueLow = 0.22;                                                              % color limitations of dot
hueHigh = 0.45;
satMin = 0.35;
valMin = 0.30;
...
while hasFrame(v)
    ...
    %% Detect neon green dot
    dotMask = (Hc > hueLow & Hc < hueHigh) & ...
              (Sc > satMin) & ...
              (Vc > valMin);
    ...
    x_pixelG(end+1,1) = x_dot;
    y_pixelG(end+1,1) = y_dot;
end

%% Put results into table
trackingData = table(frame_num, time_sec, x_pixelG, y_pixelG);
...
figure;
plot(frame_num, x_pixelG, "LineWidth", 1.5);
xlabel("Frame Number");
...
figure;
plot(frame_num, y_pixelG, "LineWidth", 1.5);
xlabel("Frame Number");
...
figure;
plot(x_pixelG, y_pixelG, "LineWidth", 1.5);
